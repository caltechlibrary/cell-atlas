...
def writePage(siteDir, source, pageName, metadata):
    ...
    subprocess.run(
        [
            ...
            "--from=markdown",
            "--to=html",
            f"--output={siteDir}/{pageName}.html",
...
def addPageToBibList(fileName, bibList, bibDict):
    ...
    if "subsections" in metadata:
        for subsectionFileName in metadata["subsections"]:
            addDocumentToBibList(
                f"subsections/{subsectionFileName}.md", bibList, bibDict
            )


def addDocumentToBibList(fileName, bibList, bibDict):
    bodyText = subprocess.check_output(
        ["pandoc", "--from=markdown", "--to=plain", fileName]
    ).decode("utf-8")
    for match in re.finditer(r"\[@.*?]", bodyText):
        bibId = match.group().strip("[@]")
        if bibDict[bibId] not in bibList:
            bibList.append(bibDict[bibId])


def getFormattedBodyText(fileName, format, bibList, bibDict):
    bodyText = subprocess.check_output(
        ["pandoc", "--from=markdown-citations", f"--to={format}", fileName]
    ).decode("utf-8")
    for match in re.finditer(r"\[@.*?]", bodyText):
        bibId = match.group().strip("[@]")
        ...
        if format == "html":
            bodyText = bodyText.replace(
                match.group(), f'[<a href="D-references.html#ref-{bibId}">{bibNum}</a>]'
            )
        elif format == "plain":
...
def addPageToSearchData(fileName, bibList, bibDict, searchData):
    ...
    createSearchDataDocument(
        fileName,
        f"{getPageName(fileName)}.html",
        bibList,
        bibDict,
        searchData,
        titlePrefix,
    ...
    if "subsections" in metadata:
        for subsectionFileName in metadata["subsections"]:
            createSearchDataDocument(
                f"subsections/{subsectionFileName}.md",
                f"{getPageName(fileName)}.html#{subsectionFileName}",
                bibList,
                bibDict,
                searchData,
...
def createSearchDataDocument(fileName, id, bibList, bibDict, searchData, titlePrefix):
    ...
    if "species" in metadata:
        document["species"] = metadata["species"]
    if "collector" in metadata:
        document["collector"] = metadata["collector"]
    if "structure" in metadata:
        document["structure"] = ", ".join(
            [structure["name"] for structure in metadata["structure"]]
        )
...
def getVidPlayerMetadata(fileName):
    ...
    if "doi" in fileMetadata:
        vidPlayerMetadata["doi"] = fileMetadata["doi"]
...
def getProteinViewerMetadata(fileName):
    ...
    for structure in fileMetadata["structure"]:
        if "PDB" in structure["name"]:
            ...
            if "modified" in structure:
                proteinViewerMetadata["modified"] = True
...
def getCitationMetadata(fileName, bibDict):
    ...
    if "collector" in fileMetadata:
        ...
        if fileMetadata["collector"] in profileData:
            ...
            citationMetadata["collectorId"] = profileData[fileMetadata["collector"]][
                "id"
            ]
    if "doi" in fileMetadata:
        citationMetadata["doi"] = fileMetadata["doi"]
    if "source" in fileMetadata:
        ...
        for source in sources:
            ...
            if id in bibDict:
                ...
                if len(bibDict[id]["author"]) > 1:
                    name = f"{name} et al."
                year = bibDict[id]["issued"]["date-parts"][0][0]
                citationMetadata["sources"].append(
                    {"text": f"{name} ({year})", "link": f"D-references.html#ref-{id}"}
                )
            elif id in profileData:
                citationMetadata["sources"].append(
                    {
                        "text": id,
                        "link": f"B-scientist-profiles.html#{profileData[id]['id']}",
                    }
                )
        if len(citationMetadata["sources"]) >= 1:
            citationMetadata["sources"][-1]["last"] = True
    if "structure" in fileMetadata:
        citationMetadata["structures"] = []
        for structure in fileMetadata["structure"]:
            if "PDB" in structure["name"]:
                structure["viewerId"] = pathlib.Path(fileName).stem
    ...
    for navEntry in navData["navList"]:
        ...
        if getPageName(fileName) == navEntry["page"]:
            ...
            for sectionEntry in navEntry["sections"]:
                if getPageName(fileName) == sectionEntry["page"]:
                    progressMetadata["currentPageNum"] = sectionEntry["pageNum"]
    progressMetadata["totalPages"] = navData["totalPages"]
    progressMetadata["progPercent"] = (
        progressMetadata["currentPageNum"] / navData["totalPages"]
    ) * 100
    progressMetadata["displayPercent"] = round(progressMetadata["progPercent"])
    progressMetadata["chapterPageNums"] = [
        {"progPercent": navEntry["progPercent"]}
        for navEntry in navData["navList"]
        if "isChapter" in navEntry
    ]
    ...
    if "subsections" in fileMetadata:
        for subsectionFileName in fileMetadata["subsections"]:
            ...
            if "species" in subsectionData:
                ...
                speciesEntry[
                    "title"
                ] = f"{fileMetadata['title']}: {subsectionData['title']}"
                speciesEntry[
                    "page"
                ] = f"{getPageName(fileName)}.html#{subsectionFileName}"
                if os.path.dirname(fileName) == "sections":
...
def addPageToFeatureIndex(fileName, featureIndex):
    ...
    if "features" in fileMetadata:
        ref = {
            "species": fileMetadata["species"],
            "link": sectionLink,
            "title": sectionTitle,
        }
        for feature in fileMetadata["features"]:
            addRefToFeatureIndex(feature, ref, featureIndex)
    ...
    if "subsections" in fileMetadata:
        for subsectionFileName in fileMetadata["subsections"]:
            ...
            if "features" in subsectionData:
                ref = {
                    "species": subsectionData["species"],
                    "link": subsectionLink,
                    "title": subsectionTitle,
                }
                for feature in subsectionData["features"]:
                    addRefToFeatureIndex(feature, ref, featureIndex)
...
def addMainSectionMetadata(fileName, metadata, bibDict):
    ...
    if "subsections" in metadata:
        ...
        for subsectionFileName in metadata["subsections"]:
            ...
            subsectionData["body"] = getFormattedBodyText(
                f"subsections/{subsectionFileName}.md", "html", bibList, bibDict
            )

            # Get media viewer metadata
            if (
                "doi" in subsectionData
                or "video" in subsectionData
                or "graphic" in subsectionData
            ):
                ...
                if "doi" in subsectionData or "video" in subsectionData:
                    subsectionData["mediaViewer"]["vidPlayer"] = getVidPlayerMetadata(
                        f"subsections/{subsectionFileName}.md"
                    )
...
def addSummarySectionMetadata(fileName, metadata):
    ...
    metadata["mediaViewer"]["summaryMenu"][
        f'chapter{os.path.basename(fileName).split("-")[0]}'
    ] = True
...
offlineAssetsExists = (
    ...
    and os.path.isdir("videos-480p")
    and os.path.isdir("stillimages")
    and os.path.isdir("narration")
)
...
navData["navList"].append(
    {"title": "Introduction", "page": "introduction", "pageNum": 1}
)
...
navData["navList"].append(
    {"chapter": "A", "title": "Feature Index", "page": "A-feature-index"}
)
navData["navList"].append(
    {"chapter": "B", "title": "Scientist Profiles", "page": "B-scientist-profiles"}
)
navData["navList"].append(
    {"chapter": "C", "title": "Phylogenetic Tree", "page": "C-phylogenetic-tree"}
)
navData["navList"].append(
    {"chapter": "D", "title": "References", "page": "D-references"}
)
...
for profileFileName in profileFileNames:
    ...
    profileMetadata["html"] = subprocess.check_output(
        ["pandoc", "--from=markdown", "--to=html", f"profiles/{profileFileName}"]
    ).decode("utf-8")
...
for fileName in sectionFileNames:
    addPageToSpeciesData(f"sections/{fileName}", speciesData)
addPageToSpeciesData("keep-looking.md", speciesData)

# Generate feature index data
featureIndex = {}
for fileName in sectionFileNames:
    addPageToFeatureIndex(f"sections/{fileName}", featureIndex)

# Generate bibliography data
bibDict = {
    ref["id"]: ref
    for ref in json.loads(
        subprocess.check_output(["pandoc", "--to=csljson", "AtlasBibTeX.bib"])
    )
}
bibList = []
...
for fileName in sectionFileNames:
    addPageToBibList(f"sections/{fileName}", bibList, bibDict)
...
for fileName in sectionFileNames:
    addPageToSearchData(f"sections/{fileName}", bibList, bibDict, searchData)
addPageToSearchData("outlook.md", bibList, bibDict, searchData)
addPageToSearchData("keep-looking.md", bibList, bibDict, searchData)
with open("{}/searchData.json".format(siteDirRegular), "w", encoding="utf-8") as f:
...
subprocess.run(
    [
        ...
        "--to=html",
        "--metadata=index",
        f"--output={siteDirRegular}/index.html",
...
if offlineAssetsExists:
    subprocess.run(
        [
            ...
            "--metadata=index",
            "--metadata=offline",
            f"--output={siteDirOffline}/index.html",
            "--template=templates/index.html",
            "index.md",
        ]
    )
if offlineAssetsExists:
    subprocess.run(
        [
            ...
            "--metadata=index",
            "--metadata=offline",
            f"--output={siteDirOfflineLite}/index.html",
            "--template=templates/index.html",
            "index.md",
        ]
    )
...
for i, fileName in enumerate(sectionFileNames):
    ...
    metadata["prevSection"] = (
        "introduction" if i == 0 else getPageName(sectionFileNames[i - 1])
    )
    metadata["nextSection"] = (
        "outlook"
        if i == len(sectionFileNames) - 1
        else getPageName(sectionFileNames[i + 1])
    )

    metadata["body"] = getFormattedBodyText(
        f"sections/{fileName}", "html", bibList, bibDict
    )
...
if offlineAssetsExists:
    writePageOffline(
        siteDirOffline, siteDirOfflineLite, "outlook.md", metadata["pageName"], metadata
    )
...
for i, fileName in enumerate(appendixFileNames):
    ...
    if fileName == "A-feature-index.md":
        ...
        metadata["features"] = {}
        metadata["features"]["entries"] = [
            {**entry, "template": {"feature": True}}
            for entry in sorted(
                featureIndex.values(), key=lambda entry: entry["title"].lower()
            )
        ]
    elif fileName == "B-scientist-profiles.md":
        metadata["appendixTypeProfiles"] = True
        metadata["profiles"] = {}
        metadata["profiles"]["entries"] = [
            {**profile, "template": {"profile": True}}
            for profile in profileData.values()
        ]
        ...
        metadata["mediaViewer"]["treeViewer"]["speciesList"] = [
            speciesEntry for speciesEntry in speciesData.values()
        ]
        metadata["speciesList"] = {}
        metadata["speciesList"]["entries"] = [
            {**entry, "title": entry["species"], "template": {"species": True}}
            for entry in sorted(
                speciesData.values(), key=lambda entry: entry["species"].lower()
            )
        ]
        metadata["treeViewerFsConfirmModal"] = {
            "id": "treeViewerFsConfirmModal",
            "label": "Allow Phylogenetic Tree full screen",
            "template": {"treeViewerFsConfirm": True},
        }
    ...
    if fileName == "D-references.md":
        ...
        subprocess.run(
            [
                ...
                "--to=html",
                f"--output={siteDirRegular}/{metadata['pageName']}.html",
                "--template=templates/page.html",
                "--metadata-file=metadata.json",
                "bib.json",
            ]
        ...
        if offlineAssetsExists:
            subprocess.run(
                [
                    ...
                    "--metadata=offline",
                    "--to=html",
                    f"--output={siteDirOffline}/{metadata['pageName']}.html",
                    "--template=templates/page.html",
                    "--metadata-file=metadata.json",
                    "bib.json",
                ]
        ...
        if offlineAssetsExists:
            subprocess.run(
                [
                    ...
                    "--metadata=offline",
                    "--to=html",
                    f"--output={siteDirOfflineLite}/{metadata['pageName']}.html",
                    "--template=templates/page.html",
                    "--metadata-file=metadata.json",
                    "bib.json",
                ]
        ...
        writePage(
            siteDirRegular, f"appendix/{fileName}", metadata["pageName"], metadata
        )
        if offlineAssetsExists:
            writePageOffline(
                siteDirOffline,
...
metadata["feedbackData"] = {
    "id": "feedback",
    "label": "Feedback",
    "template": {"feedback": True},
}
metadata["content"] = {}
metadata["content"]["entries"] = []
with open("about.md", "r", encoding="utf-8") as f:
    lines = f.readlines()
    contentStartIndex = [i for i, line in enumerate(lines) if line.strip() == "---"][
        1
    ] + 1
...
if offlineAssetsExists:
    writePageOffline(
        siteDirOffline, siteDirOfflineLite, "about.md", metadata["pageName"], metadata
    )
