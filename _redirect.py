...
def getPageTitle(fileName):
    ...
    if fileName != "index.md": title = f"{title} | Atlas of Bacterial and Archaeal Cell Structure"
    return title
...
if os.path.isdir(redirectDir): shutil.rmtree(redirectDir)
...
for fileName in sectionFileNames: writeRedirectPage(redirectDir, f"sections/{fileName}")
writeRedirectPage(redirectDir, "outlook.md")
writeRedirectPage(redirectDir, "keep-looking.md")
for fileName in appendixFileNames: writeRedirectPage(redirectDir, f"appendix/{fileName}")
