...
(ref:3-1) [Simkania negevensis](#tree) Collected by: [Martin Pilhofer](#martin_pilhofer) Movie DOI: [10.22002/D1.1477](https://doi.org/10.22002/D1.1477)

```{R 3-1, echo=FALSE, screenshot.alt='movie_stills/3_1.png' , fig.cap= '(ref:3-1)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_1_Snegevensis_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1477',0,'movie_stills/3_1.png')}
```

## Rod
Instead of a sphere, maybe you would like to make your cell cylindrical, like this *Cupriavidus necator*. **Rod-shaped** cells (cylinders with hemispherical caps) are a very common form for bacteria and archaea, likely because they are efficient swimmers and swarmers (more on that in Chapter 6). Starting from a sphere, imagine that you had a construction contractor who could direct where workers lay in new cell wall. Instead of random insertion, you could, say, direct them to work around a single plane. As the workers laid in more and more hoops of peptidoglycan in this region, a cylinder would form with the same diameter as the initial sphere (which would now serve as the structure's end caps).
...
Not all rod-shaped bacteria use MreB, and we are still figuring out how the shape forms in many species [More: Rod variety](#Rod_variety). For rod-shaped archaea [More: Archaeal rods](#Archaeal_rods), the surface layer plays an important role in determining the shape.

(ref:3-2) [Cupriavidus necator](#tree) Collected by: [Morgan Beeby](#morgan_beeby) Movie DOI: [10.22002/D1.1478](https://doi.org/10.22002/D1.1478)

```{R 3-2, echo=FALSE, screenshot.alt='movie_stills/3_2.png' , fig.cap= '(ref:3-2)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_2_Cnecator_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1478',0,'movie_stills/3_2.png')}
```

### More: Rod variety{-#Rod_variety}
...
(ref:3-2a) [Brucella abortus](#tree) Collected by: [Ariane Briegel](#ariane_briegel) Movie DOI: [10.22002/D1.1484](https://doi.org/10.22002/D1.1484)

```{R 3-2a, echo=FALSE, screenshot.alt='movie_stills/3_2a.png' , fig.cap= '(ref:3-2a)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_2a_Babortus_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1484',0,'movie_stills/3_2a.png')}
```

### More: Archaeal rods{-#Archaeal_rods}
...
(ref:3-2b) [Methanoregula formicica](#tree) Collected by: [Ariane Briegel](#ariane_briegel) Movie DOI: [10.22002/D1.1485](https://doi.org/10.22002/D1.1485)

```{R 3-2b, echo=FALSE, screenshot.alt='movie_stills/3_2b.png' , fig.cap= '(ref:3-2b)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_2b_Mformicica_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1485',0,'movie_stills/3_2b.png')}
```
...
(ref:3-4) [Caulobacter crescentus](#tree) Collected by: [Prabha Dias](#prabha_dias) Movie DOI: [10.22002/D1.1480](https://doi.org/10.22002/D1.1480)

```{R 3-4, echo=FALSE, screenshot.alt='movie_stills/3_4.png' , fig.cap= '(ref:3-4)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_4_Ccrescentus_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1480',0,'movie_stills/3_4.png')}
```

### Schematic: CTP synthase {-#CTP_synthase}
```{R}
knitr::include_graphics('img/schematics/3_4_1.gif')
```

[*PDB: 5U3C*](http://rcsb.org/structure/5U3C)
...
### More: Bacterial microtubules{-#Bacterial_microtubules}
...
(ref:3-6a) [Prosthecobacter vanneervenii](#tree) Collected by: [Martin Pilhofer](#martin_pilhofer) Movie DOI: [10.22002/D1.1487](https://doi.org/10.22002/D1.1487)

```{R 3-6a, echo=FALSE, screenshot.alt='movie_stills/3_6a.png' , fig.cap= '(ref:3-6a)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_6a_Pvanneervenii_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1487',0,'movie_stills/3_6a.png')}
```

## Square
So far we have focused on bacteria, but archaea hold their own in the specialized shape competition. In fact, one of the most extreme examples of maximizing surface area relative to volume comes from this archaeon, *Haloquadratum walsbyi*, which grows as thin, square tiles. *Very* thin, square tiles. This property helps keep them oriented with their broad sides to the sun, whose light they rely on for photosynthesis. Gas vesicles [More: Gas vesicles](#Gas_vesicles) keep the cells buoyant in the super-salty lakes where they live.
...
(ref:3-7) [Haloquadratum walsbyi](#tree) Collected by: [Zhuo Li](#zhuo_li) Movie DOI: [10.22002/D1.1483](https://doi.org/10.22002/D1.1483)

```{R 3-7, echo=FALSE, screenshot.alt='movie_stills/3_7.png' , fig.cap= '(ref:3-7)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_7_Hwalsbyi_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1483',0,'movie_stills/3_7.png')}
```

### More: Gas vesicles{-#Gas_vesicles}
...
(ref:3-7a) [Halobacterium salinarum](#tree) Collected by: [Ariane Briegel](#ariane_briegel) Movie DOI: [10.22002/D1.1488](https://doi.org/10.22002/D1.1488)

```{R 3-7a, echo=FALSE, screenshot.alt='movie_stills/3_7a.png' , fig.cap= '(ref:3-7a)'}
if(offline==TRUE){
library(embedvideo)
embedvideo('videos/3_7a_Hsalinarum_480p.mp4')
} else {
library(doivideo)
doivideo('10.22002/D1.1488',0,'movie_stills/3_7a.png')}
```

## Summary {#summary3}

```{R}
knitr::include_graphics('img/summaries/03_Shape.gif')
```
